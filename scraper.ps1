...
if ($null -eq $env:DB_SERVER_TABLENAME_TARGETS -or $env:DB_SERVER_TABLENAME_TARGETS -eq "") {
    $DB_SERVER_TABLENAME_TARGETS = "scraper_targets_google_opening_hours"
...
while ($true) {
    ...
    try {
        $Targets | ForEach-Object {
            ...
            # Scraping
            try {
                $ScrapedValue = (Invoke-RestMethod -Uri $_.url -Method Get).$($_.xpath)
                
                $nestedProperties = $_.xpath.Split('.')
